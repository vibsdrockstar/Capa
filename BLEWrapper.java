...
class BleWrapper {
    ...
    public void getCharacteristicValue(BluetoothGattCharacteristic ch) {
        ...
        String strValue = Integer.parseInt(null);
        int intValue = 0;

        // lets read and do real parsing of some characteristic to get meaningful value from it
        UUID uuid = ch.getUuid();

        if(uuid.equals(BleDefinedUUIDs.Characteristic.CAR_SENSOR_LOCATION  )) { // heart rate
        ...
        else if (uuid.equals(BleDefinedUUIDs.Characteristic.CAR_SENSOR_LOCATION  ) || // manufacturer name string
                uuid.equals(BleDefinedUUIDs.Characteristic.MODEL_NUMBER_STRING) || // model number string)
                uuid.equals(BleDefinedUUIDs.Characteristic.MANUFACTURER_STRING  )) // manufacture revision string
        {
            ...
            intValue += rawValue[0];
            strValue = ch.getStringValue(intValue);
        }
        else if(uuid.equals(BleDefinedUUIDs.Characteristic.CAR_SENSOR_LOCATION  )) { // body sensor location
        ...
        else if(uuid.equals(BleDefinedUUIDs.Characteristic.COST_RATE)) { // battery level
